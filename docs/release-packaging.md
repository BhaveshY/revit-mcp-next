...
The installer also writes `%LOCALAPPDATA%\RevitMcpNext\config\client-discovery.json`. It includes install paths, MCP launcher path, `revitctl` path, schema path, integration helper paths, tool names, catalog kinds, and write-operation names. Tool discovery includes compact read/analysis tools for views, sheets, schedules, schedule fields, current view, active-view elements, selection, parameter discovery, model statistics, model readiness, model context, model warnings, rooms, material quantities, catalogs, and bounded preview/apply write operations including family placement, guarded local family loading, sheet creation, view/schedule placement, schedule creation/field insertion, text notes, room tags, element tags, room/floor/grid creation, transforms, parameter writes, type changes, pinning, and guarded deletion. It does not include the auth token.
...
Check a completed evidence bundle before sharing or calling it production-ready:

```powershell
npm run evidence:check -- -EvidencePath artifacts\release-evidence\revit-mcp-next-<version>-windows-evidence-<timestamp>-<id> -Profile external-preview
npm run evidence:check -- -EvidencePath artifacts\release-evidence\revit-mcp-next-<version>-windows-evidence-<timestamp>-<id>.zip -Profile release-candidate
