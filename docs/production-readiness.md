...
- Core read tools now advertise typed MCP output schemas for status, documents, levels, current view, paged views/sheets/elements, model statistics/readiness/context, model warnings, material quantities, rooms, catalogs, generic queries, and parameter discovery. The schemas expose page fields such as `returnedCount`, `truncated`, `cursor`, `items`, `fields`, and `units` while remaining passthrough-safe for live Revit metadata. `revit.status` exposes queue diagnostics, ExternalEvent raise state, preview-token counts, and recovery hints for stalled workflows.
