...
- `npm run doctor:clients` validates generated Claude/Codex client config snippets, existing user config entries when present, stale install roots, launcher quoting, raw token leakage risk, Revit 2024-only client discovery, and MCP startup plus `tools/list` without requiring a Revit connection.
...
- `npm run smoke:revit` runs a live MCP smoke through the installed launcher against the active Revit project. It checks `revit.status`, `revit.get_current_view`, `revit.get_current_view_elements`, `revit.get_selection`, `revit.analyze_model`, `revit.get_model_readiness`, `revit.get_material_quantities`, `revit.get_rooms`, `revit.get_levels`, `revit.catalog`, `revit.query`, plus preview/apply flows for `create_level`, `create_grid`, `create_floor`, `create_wall`, optional `place_family_instance`, `create_room`, `set_parameter`, `move_element`, `rotate_element`, `copy_element`, `set_element_pinned`, and guarded `delete_element` cleanup of the copied smoke wall. The room section creates a closed room-bounding wall loop, places a room, verifies positive area, and reads it back through `revit.get_rooms`. The placement section uses `revit.catalog kind=familySymbols preset=placement` and either applies a supported door/window/furniture/equipment/fixture placement or records a bounded skip reason when the disposable model has no suitable symbols/hosts. When an alternate compatible wall type exists, it also applies `change_element_type`; release-candidate runs can require this with `-RequireTypeChange`. It can assert the expected Revit major year with `-ExpectedRevitYear` and write `smoke-summary.json` with pass/fail, document, Revit version, tool, and operation evidence through `-SummaryPath`.
...
The smoke mutates the active project. It creates a level, grid, floor, straight wall, and room-bounding wall loop; attempts supported family placement when the model has suitable symbols and hosts; places a room; reads the room back through `revit.get_rooms`; sets a writable wall instance parameter; optionally changes the wall type; moves the wall on the Y axis; verifies the reported movement; rotates the wall; copies it by an offset; then pins and unpins the copied wall. It also checks blocked duplicate-grid and pinned-move previews plus a rejected apply with a mismatched `changeSetHash`.
...
- Smoke scope: installed launcher, broker/add-in pipe auth via the launcher, `revit.status`, `revit.get_current_view`, `revit.get_current_view_elements`, `revit.get_selection`, `revit.analyze_model`, `revit.get_model_readiness`, `revit.get_material_quantities`, `revit.get_rooms`, `revit.get_levels`, `revit.catalog`, `revit.query`, `create_level`, `create_grid`, `create_floor`, `create_wall`, optional `place_family_instance`, `create_room`, `set_parameter`, optional or required `change_element_type`, `move_element`, `rotate_element`, `copy_element`, `set_element_pinned`, and `delete_element`.
