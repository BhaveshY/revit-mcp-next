...
This project is not yet a signed production release. The current repository state supports local development, staged Windows packaging for a Revit 2024-only production candidate, install diagnostics, redacted support bundle collection, and release evidence bundle generation.
...
- Read parity with the fork now covers compact current-view metadata, current-view elements, selected elements, model statistics, material quantities, and room export data through `revit.get_current_view`, `revit.get_current_view_elements`, `revit.get_selection`, `revit.analyze_model`, `revit.get_material_quantities`, and `revit.get_rooms`.
- Room support is implemented in the MCP/add-in surface: `revit.get_rooms` returns compact, paginated room data, and guarded `create_room` preview/apply can place a room by level and 2D location with optional name, number, department, and duplicate-number override. Live room smoke evidence is still pending.
- `npm run smoke:revit` runs a live MCP smoke through the installed launcher against the active Revit project. It checks `revit.status`, `revit.get_current_view`, `revit.get_current_view_elements`, `revit.get_selection`, `revit.analyze_model`, `revit.get_material_quantities`, `revit.get_rooms`, `revit.get_levels`, `revit.catalog`, `revit.query`, plus preview/apply flows for `create_level`, `create_grid`, `create_floor`, `create_wall`, `create_room`, `set_parameter`, `move_element`, `rotate_element`, `copy_element`, `set_element_pinned`, and guarded `delete_element` cleanup of the copied smoke wall. The room section creates a closed room-bounding wall loop, places a room, verifies positive area, and reads it back through `revit.get_rooms`. When an alternate compatible wall type exists, it also applies `change_element_type`; release-candidate runs can require this with `-RequireTypeChange`. It can assert the expected Revit major year with `-ExpectedRevitYear` and write `smoke-summary.json` with pass/fail, document, Revit version, tool, and operation evidence through `-SummaryPath`.
...
- Packaged installs include pyRevit and Dynamo status, preview/apply write examples, and host-smoke evidence examples, along with external-stdio and in-process Python helpers under `integrations`. The installer writes token-safe `config\client-discovery.json` for clients.
...
- Live room evidence for `revit.get_rooms` and guarded `create_room` against a disposable Revit 2024 model. The smoke script now exercises the room path, but current release-candidate evidence still needs to be captured from a real installed package.
...
The smoke mutates the active project. It creates a level, grid, floor, straight wall, and room-bounding wall loop; places a room; reads the room back through `revit.get_rooms`; sets a writable wall instance parameter; optionally changes the wall type; moves the wall on the Y axis; verifies the reported movement; rotates the wall; copies it by an offset; then pins and unpins the copied wall. It also checks blocked duplicate-grid and pinned-move previews plus a rejected apply with a mismatched `changeSetHash`.
...
- Smoke scope: installed launcher, broker/add-in pipe auth via the launcher, `revit.status`, `revit.get_current_view`, `revit.get_current_view_elements`, `revit.get_selection`, `revit.analyze_model`, `revit.get_material_quantities`, `revit.get_rooms`, `revit.get_levels`, `revit.catalog`, `revit.query`, `create_level`, `create_grid`, `create_floor`, `create_wall`, `create_room`, `set_parameter`, optional or required `change_element_type`, `move_element`, `rotate_element`, `copy_element`, `set_element_pinned`, and `delete_element`.
