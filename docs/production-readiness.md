...
- Unsigned local smoke builds can pause Revit on `Security - Unsigned Add-in` / `Sicherheit - Zusatzmodul ohne Signatur`. Use `Always Load` / `Immer laden` only on disposable local validation machines; production releases must be signed so end users do not see this prompt.
