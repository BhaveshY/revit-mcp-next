...
- `create_schedule`: create a non-template schedule for a supported category such as `OST_Walls`, `OST_Doors`, `OST_Rooms`, or `OST_Floors`, with optional unique `name`, initial `fields`, headings, hidden flags, and `isItemized`.
...
- `place_schedule_on_sheet`: place an existing schedule on a sheet by `sheetId`, `scheduleId`, and sheet-space `point`.
