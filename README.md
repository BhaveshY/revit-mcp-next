...
All tools return a strict MCP `structuredContent` envelope: `data`, `warnings`, `metrics`, and optional `generation`. On success, `structuredContent.data` is the typed tool payload; on bridge failure it is `{ "error": ... }`. Core read tools advertise typed output schemas through `tools/list`, including page fields such as `returnedCount`, `truncated`, `cursor`, `items`, `fields`, and `units`.
